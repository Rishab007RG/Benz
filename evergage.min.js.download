...
(function configureEvergage() {
    try {
       Evergage.configure({
           account: "daimleragapac",
           dataset: "mb_india_prod",
           cdnUrl: "https://cdn.evergage.com",
           trackerUrl: "https://daimleragapac.australia-3.evergage.com",
           siteConfigVersion: "6"
       },{"allowBotTraffic":false,"corsAllowedOrigins":["*"],"defaultCurrency":"INR","defaultLocale":null,"disableQtipWindowScroll":false,"doNotStoreCookiesRequireProvidedAnonId":false,"doNotTrackPingRequestsForActions":false,"enableCorsRestrictedOrigins":false,"enableMessageRotation":false,"enableRememberMeUserIds":false,"enableTemplateEsc":false,"hideContentSections":false,"hideContentSectionsMillis":2500,"hidePagesForRedirect":false,"hidePagesForRedirectMillis":1000,"identityAttributes":["customerId","emailAddress","profileId","sfcrmContactId","sfcrmLeadID","sfmcContactKey"],"lastModified":1670580127307,"preventSensitiveDataCapture":false,"rememberMeUserIdsMillis":63072000000,"secureCookie":false,"sendErrorEvents":true,"showPoweredBy":false,"siteConfigExecBeforePageReady":false,"spaRouteChangeTimeout":500,"trackAnonymousVisitors":true,"trackContextualRelatedItems":false,"trackSubdomainAsCompany":false,"trackUnknownPagesByTitle":false,"treatHashChangeAsPageLoad":false}       );
    } catch (e) {
          console.error(e);
    }
})();
...
/**
 *  88888888ba   88888888ba     ,ad8888ba,    88888888ba,    88        88    ,ad8888ba,  888888888888  88    ,ad8888ba,    888b      88
 *  88      "8b  88      "8b   d8"'    `"8b   88      `"8b   88        88   d8"'    `"8b      88       88   d8"'    `"8b   8888b     88
 *  88      ,8P  88      ,8P  d8'        `8b  88        `8b  88        88  d8'                88       88  d8'        `8b  88 `8b    88
 *  88aaaaaa8P'  88aaaaaa8P'  88          88  88         88  88        88  88                 88       88  88          88  88  `8b   88
 *  88""""""'    88""""88'    88          88  88         88  88        88  88                 88       88  88          88  88   `8b  88
 *  88           88    `8b    Y8,        ,8P  88         8P  88        88  Y8,                88       88  Y8,        ,8P  88    `8b 88
 *  88           88     `8b    Y8a.    .a8P   88      .a8P   Y8a.    .a8P   Y8a.    .a8P      88       88   Y8a.    .a8P   88     `8888
 *  88           88      `8b    `"Y8888Y"'    88888888Y"'     `"Y8888Y"'     `"Y8888Y"'       88       88    `"Y8888Y"'    88      `888
 */

Evergage.init({
  cookieDomain: 'mercedes-benz.co.in',
}).then(() => {
  /**
   *  I. Core utilities section
   */

  //01/ Old Model Mapping /START
  const eqArray = ['2437', '2938', '4478', '4476', '2951'];
  const hybridArray = [
    '1770861',
    '1771861',
    '2470861',
  ...
  const modelCodeToBaumusterId = (modelCode) => {
    if (modelMapping[modelCode.toUpperCase()]) return modelMapping[modelCode.toUpperCase()];
    if (modelCode.length < 4) return modelCode;
    if (modelCode.length > 0) {
      let letterModelCode = modelCode.charAt(0).toLowerCase();
      if (letterModelCode == 'w') {
        return modelCode.substring(1) + '0';
      } else if (letterModelCode == 'v') {
        return modelCode.substring(1) + '1';
      } else if (letterModelCode == 'x') {
        return modelCode.substring(1) + '6';
      } else if (letterModelCode == 'c') {
        return modelCode.substring(1) + '3';
      } else if (letterModelCode == 'a') {
        return modelCode.substring(1) + '4';
      } else if (letterModelCode == 's') {
        return modelCode.substring(1) + '2';
      } else if (letterModelCode == 'h') {
        return modelCode.substring(1) + '7';
      } else if (letterModelCode == 'n') {
        return modelCode.substring(1) + '8';
      } else if (letterModelCode == 'r') {
        return modelCode.substring(1) + '4';
      }
    }
  };
  ...
  //02/ findProductDetailDataLayer function /START
  const findProductDetailDataLayer = (frequence_check = 500, number_check_max = 50) => {
    let intervalCount = 0;
    return new Promise((resolve, reject) => {
      let interval = setInterval(() => {
        ...
        if (
          dataLayer_ow !== null &&
          typeof dataLayer_ow !== 'undefined' &&
          typeof dataLayer_ow
            .slice()
            .reverse()
            .find((k) => k.page) !== 'undefined' &&
          typeof dataLayer_ow
            .slice()
            .reverse()
            .find((k) => k.page).page.products !== 'undefined' &&
          dataLayer_ow
            .slice()
            .reverse()
            .find((k) => k.page).page.products.length == 1
        ) {
          clearInterval(interval);
          resolve(
            dataLayer_ow
  ...
  //03/ Find DataLayer functions // findInDataLayer, findBaumusterIdDataLayer, findDataLayer /START
  const findInDataLayer = (targetAttribute) => {
    if (!window.dataLayer_ow) {
      return;
    }
    for (let i = 0; i < window.dataLayer_ow.length; i++) {
      const result = Evergage.util.getValueFromNestedObject('window.dataLayer_ow[' + i + ']');
  ...
  const findBaumusterIdDataLayer = (attribute_datalayer, frequence_check = 250, number_check_max = 100) => {
    let intervalCount = 0;
    return new Promise((resolve, reject) => {
      let interval = setInterval(() => {
        intervalCount += 1;
        if (
          ...
          typeof dataLayer_ow
            .slice()
            .reverse()
            .find((k) => k.page).page.vehicle !== 'undefined' &&
          typeof dataLayer_ow
            .slice()
            .reverse()
            .find((k) => k.page).page.vehicle.baumusterId !== 'undefined'
        ) {
          clearInterval(interval);
          resolve(
            dataLayer_ow
  ...
  const findDataLayer = (attribute_datalayer, frequence_check = 200, number_check_max = 400) => {
    let intervalCount = 0;
    return new Promise((resolve, reject) => {
      let interval = setInterval(() => {
        intervalCount += 1;
        if (
          ...
          typeof dataLayer_ow
            .slice()
            .reverse()
            .find((k) => k[attribute_datalayer]) !== 'undefined'
        ) {
          clearInterval(interval);
          resolve(
            dataLayer_ow
  ...
  //04/ findConfigSaveInDataLayer /START
  const findConfigSaveInDataLayer = (frequence_check = 200, number_check_max = 400) => {
    let intervalCount = 0;
    return new Promise((resolve, reject) => {
      let interval = setInterval(() => {
        intervalCount++;

        if (dataLayer_ow !== null && typeof dataLayer_ow !== 'undefined') {
          window.dataLayer_ow.forEach((layer) => {
            if (
              typeof layer.eventDetail !== 'undefined' &&
              typeof layer.eventDetail.label !== 'undefined' &&
              layer.eventDetail.label == 'save_to_account'
            ) {
              resolve(layer);
            }
          });
  ...
  //48/ findNbaeqsDataLayer function definition /START
  const findNbaeqsDataLayer = (frequence_check = 200, number_check_max = 400) => {
    let intervalCount = 0;
    return new Promise((resolve, reject) => {
      let interval = setInterval(() => {
        intervalCount += 1;
        ...
        if (dataLayer_ow !== null && typeof dataLayer_ow !== 'undefined') {
          window.dataLayer_ow.forEach((layer) => {
            if (typeof layer.eventDetail !== 'undefined' && typeof layer.eventDetail.targetUserInformation !== 'undefined') {
              resolve(layer);
            }
          });
        } else if (intervalCount > number_check_max) {
  ...
  //06/ buildLocale /START
  const buildLocale = () => {
    if (dataLayer_ow) {
      let site = dataLayer_ow
        .slice()
        .reverse()
        .find(({ event }) => event === 'global');
      if (site) {
        let country = site.site.market;
        let lang = site.site.language;
        if (lang == 'fr') return 'fr_FR'; //%/ variable depending on market
  ...
  //08/ getCarEngineType /START
  const getCarEngineType = (baumusterID) => {
    let baumusterID4Digit = baumusterID.substring(0, 4);
    if (eqArray.indexOf(baumusterID4Digit) > -1) {
      return 'ELECTRIC';
    } else if (hybridArray.indexOf(baumusterID) > -1) {
      return 'HYBRID';
    } else {
      return 'COMBUSTOR';
    }
  ...
  //36/ pageDelay function /START
  const pageDelay = (miliseconds) => new Promise((resolve) => setTimeout(resolve, miliseconds));
  //36/ END
  ...
  let pageDetails;
  const setPageDetailsFromDataLayer = (det) => {
    pageDetails = det;
  };
  const config = {
    global: {
    ...
    pageTypes: [
      //$/ home
      {
        name: 'home',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDet) => {
              setPageDetailsFromDataLayer(pageDet);
              if (typeof pageDetails == 'undefined' || !pageDetails || !pageDetails.page) return false;
              return pageDetails.page.type.match(/home.*/) !== null;
            })
            .catch((e) => {
              return false;
            });
      ...
      //$/ product_information
      {
        ...
        isMatch: () => {
          return findDataLayer('page').then((pageDetails) => {
            setPageDetailsFromDataLayer(pageDetails);
            var pageType = pageDetails.page.type;
            var pageName = pageDetails.page.name;
            var baumusterID;
            var modelCode;
            if (pageDetails.page.vehicle) {
              baumusterID = pageDetails.page.vehicle.baumusterId;
              modelCode = pageDetails.page.vehicle.modelCode;
            }
            if (
              ((pageType == 'product' || pageType == 'landingpage' || pageType == 'generic-product-template') &&
                (typeof baumusterID !== 'undefined' || typeof modelCode !== 'undefined')) ||
              (pageType == 'generic-product-template' && pageName == 'overview')
            ) {
              var baumusterID = pageDetails.page.vehicle.baumusterId;

              //47/ baumusterID to modelCode /START
              if (typeof baumusterID == 'undefined') {
                baumusterID = modelCode;
              }
              //47/ END

              //12/ owcVerticalNavigationComponent tracking /START
              let owcVerticalNavigationComponent = document.querySelector('owc-vertical-navigation');
              if (owcVerticalNavigationComponent != null && baumusterID && typeof baumusterID !== 'undefined') {
                let owcVerticalNavigationItems = owcVerticalNavigationComponent.shadowRoot.querySelectorAll('.owc-vertical-navigation__item');
                if (owcVerticalNavigationItems.length > 0) {
                  owcVerticalNavigationItems.forEach((navElement) => {
                    let innerTextValue = navElement.querySelector('.owc-vertical-navigation__text').innerText;
                    navElement.addEventListener('click', (el) => {
                      Evergage.sendEvent({
                        action: 'PI navigation click - ' + innerTextValue + ' - ' + baumusterID,
                      });
                      console.log('PI navigation click - ' + innerTextValue + ' - ' + baumusterID);
                    });
                  });
                }
              }
              ...
              //13/ PI AEM form open tracking /START
              let wbRadioControlComponents = document.querySelectorAll('wb-radio-control');
              if (wbRadioControlComponents.length > 0) {
                wbRadioControlComponents.forEach((element) => {
                  element.addEventListener('click', () => {
                    window.isAemFormStartedEventSentToIS = false;
                    var waitForAemFormStartedEventInDatalayerInterval = setInterval(() => {
                      if (
                        typeof dataLayer_ow
                          .slice()
                          .reverse()
                          .find((k) => k.event) != 'undefined' &&
                        typeof dataLayer_ow
                          .slice()
                          .reverse()
                          .find((k) => k.event).eventDetail != 'undefined' &&
                        dataLayer_ow
                          .slice()
                          .reverse()
                          .find((k) => k.event).eventDetail.label == 'form_engaged' &&
                        !window.isAemFormStartedEventSentToIS
                      ) {
                        Evergage.sendEvent({
                          action:
                            'PI AEM Form Started ' +
                            dataLayer_ow
                              .slice()
                              .reverse()
                              .find((k) => k.event).eventDetail.vehicle.baumusterId,
                        });

                        window.isAemFormStartedEventSentToIS = true;

                        clearInterval(waitForAemFormStartedEventInDatalayerInterval);
                      } else if (window.isAemFormStartedEventSentToIS) {
                        clearInterval(waitForAemFormStartedEventInDatalayerInterval);
                      }
                    }, 100);
                  });
                ...
                //14/ PI AEM form sent tracking /START

                //NOT TESTED, form not working on BE, impossible to test

                let wbSubmitButton = document.querySelector(".multistepform button[type='submit']");
                wbSubmitButton.addEventListener('click', () => {
                  var waitForAemFormSubmitEventInDatalayerInterval = setInterval(() => {
                    if (
                      typeof dataLayer_ow
                        .slice()
                        .reverse()
                      ...
                      dataLayer_ow
                        .slice()
                        .reverse()
                        .find((k) => k.event).eventDetail.label == 'form_submitted'
                    ) {
                      Evergage.sendEvent({
                        action:
                          'PI AEM Form Completed ' +
                          dataLayer_ow
                            .slice()
                            .reverse()
                            .find((k) => k.event).eventDetail.vehicle.baumusterId,
                      ...
                      clearInterval(waitForAemFormSubmitEventInDatalayerInterval);
                    }
                  }, 100);
                });
              ...
              //37/ pageDelay function usage - 15s on PI page action /START
              pageDelay(15000).then(() => {
                if (typeof window.is15sOnPIPageEventSent == 'undefined') {
                  window.is15sOnPIPageEventSent = true;
                  Evergage.sendEvent({ action: '15 Seconds on PI Page - ' + baumusterID });
                }
              });
              //37/ END

              //16/ PI scroll 25 /START
              let checkDataLayerForPIScroll25Event = setInterval(() => {
                for (var i = 0; i < dataLayer_ow.length; i++) {
                  if (
                    typeof dataLayer_ow[i] != 'undefined' &&
                    typeof dataLayer_ow[i].event != 'undefined' &&
                    dataLayer_ow[i].event == 'target_dmp_scroll' &&
                    typeof dataLayer_ow[i].eventdetail != 'undefined' &&
                    typeof dataLayer_ow[i].eventdetail.scrollingdepth != 'undefined'
                  ) {
                    if (dataLayer_ow[i].eventdetail.scrollingdepth == 25) {
                      clearInterval(checkDataLayerForPIScroll25Event);

                      if (typeof window.isPIScroll25EventSent == 'undefined') {
                        window.isPIScroll25EventSent = true;
                        Evergage.sendEvent({
                          action: 'PI Scroll 25: ' + baumusterID,
                        });
                      }
                    }
                  }
              ...
              //16/ END
              //40/ PI scroll 50 /START
              let checkDataLayerForPIScroll50Event = setInterval(() => {
                for (var i = 0; i < dataLayer_ow.length; i++) {
                  if (
                    typeof dataLayer_ow[i] != 'undefined' &&
                    typeof dataLayer_ow[i].event != 'undefined' &&
                    ...
                    if (dataLayer_ow[i].eventdetail.scrollingdepth == 50) {
                      clearInterval(checkDataLayerForPIScroll50Event);

                      if (typeof window.isPIScroll50EventSent == 'undefined') {
                        window.isPIScroll50EventSent = true;
                        Evergage.sendEvent({
                          action: 'PI Scroll 50: ' + baumusterID,
                        });
                      }
                    }
                  }
        ...
        action: () => {
          ...
          //54/ datalayer pagename action PI page /START
          Evergage.sendEvent({ action: 'Product Information Page - ' + pageName + ' ' + baumusterID });
          //54/ END

          //45/ catalogTopic ItemClass action send /START
          var catalogTopic = pageDetails.page.name;

          let navHref = document?.querySelector('owc-vertical-navigation')?.shadowRoot?.querySelector('.owc-vertical-navigation__item--active')
            ?.firstChild.href;
          if (navHref) {
            catalogTopic = navHref.substr(navHref.indexOf('#') + 1);
          }

          //52/ catalogTopic matching to desired affinities /START
          if (catalogTopic == 'exterior' || catalogTopic == 'interior') {
            catalogTopic = 'Design';
          }
          if (catalogTopic == 'technical-data' || catalogTopic == 'technology') {
            catalogTopic = 'Technology';
          }
          if (catalogTopic == 'mercedes-amg') {
            catalogTopic = 'AMG';
          }
          //52/ /END
          ...
          Evergage.sendEvent({
            ...
            catalog: {
              ItemClass: { _id: catalogTopic },
            },
          });
          //45/ END

          if (pageName && baumusterID && typeof baumusterID !== 'undefined') {
            //19/ EQS PI page tracking on scrolling behaviour /START
            if (baumusterID == '2971') {
              var getModuleFromUrl = () => {
                var idx = window.location.href.indexOf('.module.html');
                for (i = idx; i >= 0; i--) {
                  if (window.location.href[i] == '/') {
                    return window.location.href.substr(i + 1, idx - i - 1);
                  }
                }
              };

              var moduleFromUrl = getModuleFromUrl();
              if (typeof moduleFromUrl != 'undefined') {
                return 'Product Information Page EQS - ' + moduleFromUrl + ' ' + baumusterID;
              } else {
                return 'Product Information Page EQS - ' + 'pad' + ' ' + baumusterID;
              }
            }
            //19/ END
            else {
              //46/ catalogTopic usage in action /START
              return 'Product Information Page - ' + catalogTopic + ' ' + baumusterID;
              //46/ END
            }
          }
        },
        ...
        catalog: {
          Product: {
            _id: () => {
              var baumusterID = pageDetails.page.vehicle.baumusterId;
              var modelCode = pageDetails.page.vehicle.modelCode;
              if (baumusterID) {
                return baumusterID;
              } else if (modelCode) {
                return modelCodeToBaumusterId(modelCode);
              }
            },
            dimensions: {
              BaumusterShort: () => {
                if (
                  typeof dataLayer_ow
                    .slice()
                    .reverse()
                    .find((k) => k.page).page.vehicle.baumusterId == 'string'
                ) {
                  var baumusterId = dataLayer_ow
                    .slice()
                    .reverse()
                    .find((k) => k.page).page.vehicle.baumusterId;
                  var baumusterShort = baumusterId.substr(0, 4);
                  var baumusterShortArray = [baumusterShort];
                  return baumusterShortArray;
                }
              },
            },
          },
        ...
        // Listeners - old listener solution, currently not working
        listeners: [
          Evergage.listener('click', "a[href*='0.pricelist']", (e) => {
            return findDataLayer('page').then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              var modelName = pageDetails.page.vehicle.modelName;
              Evergage.sendEvent({
                action: 'Download Pricelist ' + modelName,
              });
            });
          }),
          Evergage.listener('click', "a[href*='2.brochure']", (e) => {
            return findDataLayer('page').then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              var modelName = pageDetails.page.vehicle.modelName;
              Evergage.sendEvent({
                action: 'Download Brochure ' + modelName,
              });
            });
          }),
          Evergage.listener('click', "a[href*='1.brochure']", (e) => {
            return findDataLayer('page').then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              var modelName = pageDetails.page.vehicle.modelName;
              Evergage.sendEvent({
          ...
          Evergage.listener('click', '.livechat', () => {
            return findDataLayer('page').then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              var modelName = pageDetails.page.vehicle.modelName;
              if (modelName) {
                Evergage.sendEvent({
                  action: 'Live-chat ' + modelName,
                });
              }
            });
          }),
          Evergage.listener('click', "a[href*='dealer-locator']", (e) => {
            return findDataLayer('page').then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              var modelName = pageDetails.page.vehicle.modelName;
              if (modelName) {
                Evergage.sendEvent({
                  action: 'Dealer Locator from ' + modelName,
                });
              }
            });
          }),
          Evergage.listener('click', "a[href*='online-testdrive']", (e) => {
            return findDataLayer('page').then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              var modelName = pageDetails.page.vehicle.modelName;
              if (modelName) {
                Evergage.sendEvent({
                  action: 'Test Drive from ' + modelName,
                });
              }
            });
          }),
      ...
      //$/ car_configurator_default
      {
        name: 'car_configurator_default',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              ...
              var pageName = pageDetails.page.name;
              return (
                (pageType == 'carconfigurator' || pageType == 'configuration' || pageType == 'vmoscs' || pageName == 'configurator') &&
                pageName !== 'select_motorization' &&
                pageName !== 'your_vehicle'
              );
            })
            .catch((e) => {
              return false;
        ...
        action: 'Car Configurator Default Page',
      },

      //$/ car_configurator_start
      {
        name: 'car_configurator_start',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              ...
              //50/ pageDelay function usage - Evergage.reinit on car configuration page /START
              if (pageName == 'car-configurator' && pageType == 'generic-template') {
                pageDelay(1000).then(() => {
                  Evergage.reinit();
                });
              }
              //50 /END
              else if (pageType == 'configuration' || pageName == 'car-configurator' || pageName == 'configurator') {
                return findBaumusterIdDataLayer('page')
                  .then((pageDetails) => {
                    setPageDetailsFromDataLayer(pageDetails);
                    var pageType = pageDetails.page.type;

                    if (pageName == 'select_motorization') {
                      Evergage.sendEvent({
                        action: 'Car Configuration Started',
                      });
        ...
        action: () => {
          var baumusterID = pageDetails.page.vehicle.baumusterId;
          if (baumusterID && typeof baumusterID !== 'undefined') {
            let engineType = getCarEngineType(baumusterID);
            var shortBaumusterID = baumusterID.substring(0, 4);
            return 'Car Configuration Started - ' + engineType + ' - ' + shortBaumusterID;
          }
        },
        catalog: {
          Product: {
      ...
      //$/ car_configurator_select
      {
        name: 'car_configurator_select',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              ...
              return pageType == 'generic-template' && pageName == 'configurator';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Car Configurator - vehicle selection',
      },

      //$/ car_configurator_finish
      {
        name: 'car_configurator_finish',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              ...
              if (pageType == 'configuration') {
                ...
                window.formStartedActionSent = false;
                window.formFinishedActionSent = false;
                ...
                return findBaumusterIdDataLayer('page')
                  .then((pageDetails) => {
                    ...
                    if (pageType == 'configuration' && pageName == 'your_vehicle') {
                      // Evergage.sendEvent({
                      //     action: "Car Configuration Finished"
                      // })

                      //18/ findConfigSaveInDataLayer usage /START
                      findConfigSaveInDataLayer().then((layer) => {
                        Evergage.sendEvent({
                          action: 'Car Configuration Saved',
                        });
                        Evergage.sendEvent({
                          action: 'Car Configuration Saved - ' + layer.eventDetail.vehicle.modelCode,
                        });
                      });
                      //18/ END

                      //51/ new offer request tracking /START
                      var waitForFormStartEventInDataLayer = setInterval(() => {
                        ...
                        if (
                          ...
                          typeof dataLayer_ow
                            .slice()
                            .reverse()
                            .find((k) => k.event).eventDetail.label != 'undefined' &&
                          dataLayer_ow
                            .slice()
                            .reverse()
                            .find((k) => k.event).eventDetail.label == 'form_started'
                        ) {
                          clearInterval(waitForFormStartEventInDataLayer);

                            if (!window.formStartedActionSent) {
                            Evergage.sendEvent({
                                action: 'Offer request form started',
                            });

                            var baumusterShort = dataLayer_ow
                                .slice()
                                .reverse()
                                .find((k) => k.page)
                                .page.vehicle.baumusterId.substr(0, 4);
                            Evergage.sendEvent({
                                action: 'Offer request form started - ' + baumusterShort,
                            });

                            window.formStartedActionSent = true
        
                            }
                        }
                      }, 500);

                      var waitForFormSubmitEventInDataLayer = setInterval(() => {
                        //form submitted
                        if (
                          typeof dataLayer_ow
                            .slice()
                          ...
                          dataLayer_ow
                            ...
                            clearInterval(waitForFormSubmitEventInDataLayer);

                            if (!window.formFinishedActionSent) {

                            Evergage.sendEvent({
                                action: 'Offer request form finished',
                            });
                            ...
                            var baumusterShort = dataLayer_ow
                                ...
                                .page.vehicle.baumusterId.substr(0, 4);
                            Evergage.sendEvent({
                                action: 'Offer request form finished - ' + baumusterShort,
                            });
                            window.formFinishedActionSent = true
                            }
                        }
        ...
        action: () => {
          ...
          if (baumusterID && typeof baumusterID !== 'undefined') {
            ...
            return 'Car Configuration Finished - ' + engineType + ' - ' + shortBaumusterID;
          }
        },
        catalog: {
          Product: {
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              return pageType == 'product' && pageName == 'foerderungen';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'EV Subsidiation',
      },
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              return pageType == 'product' && pageName == 'charging-services';
            })
            .catch((e) => {
              return false;
            });
      ...
      {
        name: 'eq_ready_app',
        isMatch: () => /\/eq-ready-teaser/.test(window.location.href),
        action: 'EQ ready APP',
      },

      //$/ e_mobility_overview
      {
        name: 'e_mobility_overview',
        isMatch: () => {
          return findDataLayer('page').then((pageDetails) => {
            setPageDetailsFromDataLayer(pageDetails);
            var pageType = pageDetails.page.type;
            var pageName = pageDetails.page.name;
            return pageType == 'product' && pageName == 'e-mobility';
          });
        },
        action: 'E Mobility Overview Page',
      },

      //$/ eq_overview_all
      {
        name: 'eq_overview_all',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              return pageType == 'product' && pageName == 'eq-power';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'EQ Overview Page',
      },

      //$/ hybrid_overview_all
      {
        name: 'hybrid_overview_all',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              return pageType == 'product' && pageName == 'plug-in-hybrid';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Plug-in Hybrid Overview Page',
      },

      //$/ used_cars_page
      {
        name: 'used_cars_page',
        isMatch: () => /\/used-cars/.test(window.location.href),
        action: 'Used Cars Page',
      },

      //$/ used_cars_search
      {
        name: 'used_cars_search',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              return pageName == 'vehicle-search' && pageType == 'vehiclesearch';
            })
            .catch((e) => {
              return false;
            });
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              return pageType == 'product' && pageName == 'financing';
            })
            .catch((e) => {
              return false;
            });
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              return pageName == 'leasing' && pageType == 'product';
            })
            .catch((e) => {
              return false;
            });
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              return pageType == 'landingpage' && pageName == 'sustainability';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Sustainability Hub Page',
      },
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              return pageType == 'dealerlocator' || pageName == 'dealer-locator';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Dealer Locator Page',
      },
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              return pageName == 'product_list.cars' || pageName == 'search-results';
            })
            .catch((e) => {
              return false;
            });
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              if ((pageType == 'product_detail' && pageName == 'product_detail') || pageName == 'product') {
                //43/ findProductDetailDataLayer function (//02/) usage /START
                return findProductDetailDataLayer().then((pageDet) => {
                  setPageDetailsFromDataLayer(pageDet);
                  var pageType = pageDet.page.type;
                  var pageName = pageDet.page.name;
                  if (pageType == 'product_detail' && pageName == 'product_detail') {
                    Evergage.sendEvent({
                      action: 'Product Detail Page - ' + pageDet.page.products[0].id,
                    });

                    //30/ Quote request tracking /START
                    var emhProductDetailsComponent = document.querySelector('emh-product-details').shadowRoot;
                    emhProductDetailsComponent.querySelector('button.dcp-cars-buy-box__main-cta').addEventListener('click', () => {
                      Evergage.sendEvent({
                        action: 'Quote Request Started',
                      });

                      var waitForFormOpenEventIntervalCount = 0;
                      var waitForFormOpenEventInterval = setInterval(() => {
                        waitForFormOpenEventIntervalCount++;
                        ...
                        if (
                          ...
                          dataLayer_ow
                            ...
                            .find((k) => k.event).eventDetail.label == 'form_started' &&
                          document
                            .querySelector('emh-product-details')
                            .shadowRoot.querySelector('aemf-form-container')
                            .shadowRoot.querySelectorAll("button[type='submit'][class*='primary']").length > 0
                        ) {
                          var aemfFormContainerComponent = emhProductDetailsComponent.querySelector('aemf-form-container').shadowRoot;
                          aemfFormContainerComponent.querySelector("button[type='submit'][class*='primary']").addEventListener('click', () => {
                            var waitForFormSubmitEventIntervalCount = 0;
                            var waitForFormSubmitEventInterval = setInterval(() => {
                              waitForFormSubmitEventIntervalCount++;
                              ...
                              if (
                                ...
                                Evergage.sendEvent({
                                  action: 'Quote Request Completed',
                                });
                                clearInterval(waitForFormSubmitEventInterval);
                              }
                            }, 100);
                          });

                          clearInterval(waitForFormOpenEventInterval);
                        }
                      }, 100);
                    });
                    //30/ END
      ...
      //$/ me_dashboard
      {
        name: 'me_dashboard',
        isMatch: () => {
          //49/ findNbaeqsDataLayer function usage /START
          findNbaeqsDataLayer().then((layer) => {
            Evergage.sendEvent({
              user: {
                attributes: {
                  nbaEQS: layer.eventDetail.targetUserInformation.nextStep,
                },
              },
            });
          });
          ...
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              return pageName == 'my-dashboard';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Mercedes Me Dashboard Page',
      },
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              if (pageType == 'myVehicle' && pageName == 'my-vehicle') {
                return true;
              }
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              return pageName != 'my-dashboard' && /\/my-area/.test(window.location.href);
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Mercedes Me Area',
      },
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              return pageType == 'oab' && pageName == 'oab.confirmation';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Dealer appointment - Service Appointment',
      },
      //$/ downloads
      {
        name: 'downloads',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              return pageType == 'landingpage' && pageName == 'pricelist-brochures';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Download Pricelists and Brochures Page',
        listeners: [
          Evergage.listener('click', "a[href*='0.pricelist']", (e) => {
            let currentUrl = window.location.href;
            let urlEnd = currentUrl.split('/').pop();
            let modelName = urlEnd.replace('.module.html', '');
            Evergage.sendEvent({
              action: 'Download Pricelist ' + modelName,
            });
          }),
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              return pageType == 'landingpage' && pageName == 'design';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Design page visit',
      },
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              return pageType == 'product' && pageName == 'finance';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Finance page visit',
      },
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              return pageType == 'landingpage' && pageName == 'mercedes-amg';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'AMG page visit',
      },

      //$/ technology_page
      {
        name: 'technology_page',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              ...
              return pageType == 'landingpage' && pageName == 'intelligent-drive';
            })
            .catch((e) => {
              return false;
            });
        },
        action: 'Technology page visit',
      },

      //$/ contact_multi_select_form
      {
        name: 'contact_multi_select_form',
        isMatch: () => {
          return findDataLayer('page').then((pageDetails) => {
            setPageDetailsFromDataLayer(pageDetails);
            var pageName = pageDetails.page.name;

            //34/ AEM form tracking on multi select form page (started and completed) /START
            if (pageName == 'contact-multi-select') {
              //aem form start tracking
              let wbRadioControlComponents = document.querySelectorAll('wb-radio-control');
              if (wbRadioControlComponents.length > 0) {
                wbRadioControlComponents.forEach((element) => {
                  element.addEventListener('click', () => {
                    ...
                    var waitForAemFormStartedEventInDatalayerInterval = setInterval(() => {
                      if (
                        ...
                        if (!window.isAemFormStartedEventSentToIS) {
                          window.isAemFormStartedEventSentToIS = true;
        ...
        action: () => {
          ...
          return 'Contact multi select form';
        },
      },
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              var pageType = pageDetails.page.type;
              var pageName = pageDetails.page.name;
              //44/ pageDelay function usage - Evergage.reinit on partially matched page
              if (pageName == 'online-testdrive' && pageType == 'generic-template') {
                pageDelay(1000).then(() => {
                  Evergage.reinit();
                });
              }
              ...
              if (pageType == 'test_drive' && pageName == 'dealer') {
                ///
                var URL = window.location.href;
                var modelCode = URL.substring(URL.indexOf('Series=') + 7, URL.indexOf('Series=') + 11);
                if (typeof modelCode != 'undefined') {
                  Evergage.sendEvent({
                    action: 'Test Drive Started - ' + modelCode,
                  });
                }
      ...
      //$/ test_drive_schedule
      {
        ...
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              ...
              if (pageType == 'test_drive' && pageName == 'schedule') {
                return true;
              }
              return false;
            })
        ...
        action: 'Test Drive Schedule',
      },
      //$/ test_drive_personal_data
      {
        name: 'test_drive_personal_data',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              ...
              if (pageType == 'test_drive' && pageName == 'personal_data') {
                return true;
              }
              return false;
            })
        ...
        action: 'Test Drive Personal Data',
      },
      //$/ test_drive_finish
      {
        name: 'test_drive_finish',
        isMatch: () => {
          return findDataLayer('page')
            .then((pageDetails) => {
              setPageDetailsFromDataLayer(pageDetails);
              ...
              if (pageType == 'test_drive' && pageName == 'confirmed') {
                ///
                ...
                Evergage.sendEvent({
                  action: 'Test Drive Booked - ' + modelCode,
                });

                //26/ Test Drive Booked - A-Class action /START
                if (modelCode === 'V177' || modelCode === 'W177') {
                  Evergage.sendEvent({
                    action: 'Test Drive Booked - A-Class',
                  });
                }
                //26/ END
      ...
      //                 //41/ MILP scroll 50 /START
      //                 let checkDataLayerForPIScroll50Event = setInterval(() => {
      //                     for (var i = 0; i < dataLayer_ow.length; i++) {
      //                         if (typeof dataLayer_ow[i] != 'undefined' &&
      //                             typeof dataLayer_ow[i].event != 'undefined' &&
      //                             dataLayer_ow[i].event == 'target_dmp_scroll' &&
      //                             typeof dataLayer_ow[i].eventdetail != 'undefined' &&
      //                             typeof dataLayer_ow[i].eventdetail.scrollingdepth != 'undefined') {
      ...
      //                                 //"MILP Page Scroll 50:"+ bm4
      //                                 Evergage.sendEvent({
      //                                     action: "MILP Page Scroll 50: " + pageName
      //                                 })
      ...
      //                 //38/ pageDelay function usage - 15s on MILP page action /START
      //                 pageDelay(15000).then(() => {
      //                     Evergage.sendEvent({ action: "15 Seconds on MILP Page"})
      //                 });
      //                 //38/ END
  ...
  //10/ handleSPAPageChange /START
  const handleSPAPageChange = () => {
    if (
      !/\/passengercars\/buy\/new-car\/search-results.html/.test(window.location.href) &&
      !/\/passengercars\/mercedes-benz-cars\/online-testdrive\.html/.test(window.location.href) &&
      !/mercedes\-benz\.[a-z]+\.[a-z]+\/passengercars\.html/i.test(window.location.href)
    ) {
      /////ignore for dcp search results page///
      let url = window.location.href;

      let urlChangeInterval = setInterval(() => {
        if (url !== window.location.href) {
          console.log('SAP init');
          url = window.location.href;
          Evergage.reinit();
        }

        //2.3. Handle PI EVO Page Change /START
        let navHref = document?.querySelector('owc-vertical-navigation')?.shadowRoot?.querySelector('.owc-vertical-navigation__item--active')
          ?.firstChild.href;
        if (
          dataLayer_ow
            .slice()
            .reverse()
            .find((k) => k.page).page.type == 'generic-product-template' &&
          navHref !==
          document?.querySelector('owc-vertical-navigation')?.shadowRoot?.querySelector('.owc-vertical-navigation__item--active')?.firstChild.href
        ) {
          console.log('SAP AClass init');
          navHref = document?.querySelector('owc-vertical-navigation')?.shadowRoot?.querySelector('.owc-vertical-navigation__item--active')
            ?.firstChild.href;
          Evergage.reinit();
        }
        //2.3. /END
